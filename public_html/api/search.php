...
if (empty($q)) {
    sendResponse(400, "Parâmetro de busca vazio");
}
...
if ($db) {
    try {
        $query = "SELECT sp.*, c.name as category_name FROM sticker_packs sp 
                  ...
                  WHERE sp.name LIKE :q OR sp.creator LIKE :q OR c.name LIKE :q
                  ORDER BY sp.downloads_count DESC LIMIT 15";
        $stmt = $db->prepare($query);
        $stmt->execute([':q' => "%$q%"]);
        $rows = $stmt->fetchAll();
        
        foreach ($rows as $row) {
            $st_stmt = $db->prepare("SELECT * FROM stickers WHERE pack_id = :pack_id ORDER BY order_index ASC LIMIT 3");
...
if (empty($results)) {
    // Dynamic query simulation over MockData
    $mock_packs = [
        [
            "id" => "cyber_gatos",
            "name" => "Cyber Gatos",
    ...
    foreach ($mock_packs as $p) {
        if (stripos($p['name'], $q) !== false || stripos($p['creator'], $q) !== false || stripos($p['category'], $q) !== false) {
            $results[] = $p;
        }
    }
}

sendResponse(200, "Search completed successfully", $results);
